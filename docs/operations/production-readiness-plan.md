...
- [ ] Record and test hosted Auth session, refresh-token reuse, CAPTCHA, and rate-limit settings. For every email-code or link flow, verify same-origin/allowlisted redirects, short expiry (target ten minutes unless a documented usability/risk decision requires otherwise), provider-enforced one-time use, resend invalidation, and layered per-email plus per-source throttling. Measure legitimate delivery/retry behavior before adopting any fixed limit such as three requests per email per hour.
- [ ] Validate production host/origin/security headers and webhook size/signature defenses at the edge. Application configuration already sends both `X-Frame-Options: DENY` and CSP `frame-ancestors 'none'`; add an external regression check proving both headers cover HTML, auth, public-token, error, redirect, and API responses on every production hostname, and include an attempted cross-origin iframe/clickjacking test.
- [ ] Replace the current SMS-specific privacy and terms pages with counsel-reviewed, product-wide Privacy Policy and Terms of Service covering scheduling, family/student data, authentication, communications, billing, payments, retention/deletion, subprocessors, school/customer responsibilities, and incident/support contacts. Preserve any program-specific SMS disclosures separately.
- [ ] Prepare a counsel-reviewed Data Processing Agreement and subprocessor list before contracting with schools that require them. Map controller/processor roles, data categories, purposes, retention/deletion, security measures, incident notification, cross-border transfer terms, data-subject request handling, and vendor flow-down obligations; do not treat AI-generated legal text as legal approval.
- [ ] Establish a named compliance owner and recurring calendar: quarterly review at minimum, plus event-driven review after material product, provider, data-use, jurisdiction, or legal changes. Record scope, reviewer, changes, approvals, next due date, and evidence rather than relying on an arbitrary subscription to a compliance tool.
