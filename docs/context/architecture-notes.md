...
- A student has no permanent exclusive teacher relationship. A recurring lesson series supplies the usual teacher as a planning default, while every materialized lesson occurrence carries its own assigned teacher and may override that default.
- Teacher substitution is occurrence-scoped. One or several lessons may be reassigned to a substitute without rewriting the recurring series, the student's usual schedule, price snapshot, or other occurrences. Record the prior teacher, substitute teacher, actor, timestamp, and structured reason in immutable lesson-change history.
- Standalone lessons are first-class occurrences and do not require a recurring series. This covers ad-hoc lessons, irregular students, trial lessons, teacher pickup work, and makeup lessons accepted from another teacher.
- A makeup occurrence may optionally reference the original cancelled/rescheduled occurrence or makeup entitlement for policy and billing attribution, but its operational teacher, time, duration, and place remain independent occurrence facts.
- Payroll/workload reporting attributes delivered work to the occurrence's actual teacher. Student scheduling views may still describe the recurring series teacher as the usual teacher; neither view should infer exclusivity.
