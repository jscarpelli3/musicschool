...
1. Finish the 2026-09-12 pre-beta security gate: same-total statement-revision rehearsal, immutable notice retry rehearsal, remaining documentation reconciliation, and independent-review disposition. The full-program adversarial baseline remains explicitly open.
2. Publish and cold-test the implemented invitation-only school onboarding: private signup link, school basics and branding, owner avatar, atomic first student and payer, instruments, additional teachers, and plain-language billing/approval orientation.
3. After that cold-path acceptance, begin the broad UI pass.
4. Rehearse the deployed human-centered cancellation workflow: versioned policies, family late-fee preview, atomic owner request decisions, lesson accounting overrides, the `Lessons to Schedule` entitlement foundation, and durable fee/credit adjustments. Pending adjustments enter an existing editable period or wait idempotently for the next one.
5. Keep the owner planner and payment surfaces reliable on phone, touch-only tablet, keyboard, and desktop.
6. Do not implement Stripe charge execution until the exact-statement notice binding, provider failure tests, and one genuine beta statement delivery pass.

## Provider State

- **Supabase:** linked and migrated through `20260912115000`; tenant, people, scheduling, billing, exact-statement notice evidence, invitation-only onboarding, platform issuance audit, append-only decision revisions, approval, consent, and provider-event foundations are deployed with RLS and server authorization. Linked lint reports only two pre-existing unused-variable warnings in `queue_payer_response_notifications` and `resolve_owner_lesson_change_request`.
...
0. Complete the teacher flow now underway: assigned schedule, outcome entry, self-rescheduling, invitation delivery reconciliation, and invitation cooldowns are implemented. Next run teacher invitation/access acceptance, add teacher-cancellation remedy review, and complete the teacher-specific authorization/RLS matrix.
