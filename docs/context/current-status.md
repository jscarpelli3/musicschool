...
- Owner-notification manual retries are database-claimed and throttled: concurrent/stale clicks are refused, failures receive increasing cooldowns, the UI shows retry availability, and a 12-retry ceiling requires support intervention. Stable provider idempotency remains the final duplicate-send safeguard.
