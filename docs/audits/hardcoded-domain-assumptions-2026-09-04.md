...
This closes release blockers 1–3 and high-priority item 3. The next domain-flow work remains the unified rescheduling command, followed by explicit school-cancellation submission and policy-recipe conflict verification. Structural follow-up remains the generic delivery registry, capability-derived UI/actions, shared reason/status contracts, and exhaustive calendar state descriptors.
