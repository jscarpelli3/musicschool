...
This also closes high-priority item 2 for existing reschedule entry points. The next domain-flow work is explicit school-cancellation submission, followed by policy-recipe conflict verification. Structural follow-up remains the generic delivery registry, capability-derived UI/actions, shared reason/status contracts, and exhaustive calendar state descriptors.
