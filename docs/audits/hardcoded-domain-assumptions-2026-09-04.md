...
Capability conversion remains in progress for mixed-authority RPCs where role is both workflow identity and one branch of authorization, including owner/teacher rescheduling and staff administration. These require preserving assignment-specific teacher behavior while replacing only the management branch. After that, structural follow-up is shared reason/status contracts and exhaustive calendar state descriptors.
