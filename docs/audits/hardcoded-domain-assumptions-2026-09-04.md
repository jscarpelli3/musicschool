...
Migrations `20260908130000_structured_reschedule_rpc_boundary.sql` and `20260908131000_verify_structured_reschedule_rpc_boundary.sql` replace the public `code::detail` reschedule transport with independent `reason_code` and `reason_detail` RPC fields for both management and assigned-teacher origins. The Server Actions parse and validate the shared typed contract before crossing the database boundary. Authenticated clients can no longer execute the legacy encoded wrappers; those remain private compatibility adapters around the existing normalization trigger until the underlying history write is revised. Both migrations deployed successfully and linked database lint passes.

Remaining structural work is to remove that private persistence adapter and extend exhaustive descriptors to proposal, request, billing, and delivery states.
