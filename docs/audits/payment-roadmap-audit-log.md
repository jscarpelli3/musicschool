...
- Manual owner-email retry no longer relies on instructions not to click repeatedly. Migration `20260818150000` adds a database-locked retry claim, retry count, next-eligible time, escalating cooldown, 12-attempt ceiling, and audit event. The UI disables cooling-down or exhausted retries and displays the next eligible time. Accepted mail ceases to be retryable, concurrent/stale claims fail, and the stable provider idempotency key continues to protect the external side effect. Migration is deployed; TypeScript and ESLint pass; linked lint reports no new issues.
