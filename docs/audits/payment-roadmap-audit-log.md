...
- Raw bearer tokens remain intentionally unpersisted, so retry cannot reconstruct the failed email's link. Token rotation within the same request invalidates the unseen prior token without falsely creating a new financial proposal version. TypeScript, ESLint, and the webpack production build pass; migration deployment and live retry acceptance remain.
