...
- Live recovery passed: the Garcia $200 statement returned to editable review, the payer email was corrected, and the unchanged five-line statement was relocked. The next functional test is the exact `Email approval request` action.
- The rehearsal exposed explicit UI debt: the workflow needs an unmistakable sense of progression and next-action guidance, though not necessarily numbered stages; stale action feedback must clear; delivery failures must name recipient and attempt time; all consequential preparation, refresh, lock, send/replace, and charge controls require hold-to-confirm; statement context and dashboard location must remain visible; unavailable actions must be disabled with a reason; and a focused modal/sheet or dedicated statement surface should replace the ambiguous embedded box. These are recorded acceptance requirements, but functional completion remains the current priority.
- Version 2 was created for the corrected Garcia $200 statement and handed to Resend for `elscarpo@gmail.com`. The owner UI displayed `Email delivery: accepted`; this is a terminology defect because it can be mistaken for payer approval. Provider intake and payer authorization must use unmistakably different language.
- The Garcia version 2 request was approved accidentally during rejection rehearsal. This caused no charge and remains useful valid approval evidence. The payer page incorrectly left `Something looks wrong with these charges` / rejection controls active after approval. Those controls must be rendered only for a pending request; post-approval concerns are a separate contact, correction, credit, or refund workflow and may not mutate the accepted request.
- The next rejection rehearsal moved to Kim family. Its reachable payer email is saved, but August draft preparation correctly stopped because the August 13 lesson has no outcome. That exposed two prerequisites before resuming: add guarded, audited owner/assigned-teacher lesson-outcome entry, and model whether the school bills before or after service rather than implicitly requiring outcomes for every workflow.
- Billing timing must have a school default (`before service` or `after service`), an optional lesson/class configuration override, and an immutable resolved snapshot on the student's lesson agreement. Before-service drafts use upcoming scheduled obligations and reconcile later policy changes through credits/adjustments; after-service drafts require actual outcomes. Billing day, review deadline, and charge date are separate timing settings.
- Monthly statement adjustments are not sufficient for the complete financial workflow. Add an append-only family account ledger for one-off charges, granted credits, credit applications, payments, refunds, reversals, and write-offs. Every entry records amount/currency, reason/category, actor, timestamps, related student/lesson when applicable, related statement/payment/provider objects, and reversal lineage; financial history is corrected through compensating entries rather than edits or deletion.
- A granted credit creates an account-level available balance that survives billing periods. Draft preparation may propose applying some or all eligible credit, but the application becomes a distinct ledger allocation bound to the statement and must be released if that statement is revised or voided. The system must prevent double application and expose available, reserved, applied, and reversed amounts.
- A one-off statement credit remains useful when it belongs only to the statement under review. An account credit is used when value must be held for future use. The UI must name these differently so owners do not accidentally create a persistent balance when they only mean to correct one proposal.
- One-off charges may be added to an editable statement or created as a standalone account charge. Standalone collection still requires the applicable payer approval or active automatic-charge mandate; recording a charge is never equivalent to collecting it.
- Refunds are separate from credits. A refund references a settled payment and returns money through the provider, while an account credit retains value at the school for future statements. Partial refunds, failed/pending provider states, and refund reversals must reconcile through webhooks before local truth is marked settled.
- Add receipt retrieval to owner and payer history. Persist the connected-account ID, PaymentIntent ID, Charge ID, provider receipt number when present, and local payment/statement relationships—not Stripe's expiring hosted URL. An authorized server endpoint retrieves the Charge in the correct connected-account scope and redirects to its current `receipt_url`; access is tenant- and payer-scoped, rate-limited, and audited.
- Receipts belong to payments, not intrinsically to one student. A family payment may cover siblings or account-level charges. Student detail pages may list receipts whose immutable statement allocation includes that student, but must label a multi-student payment as a family receipt and show the relevant allocation rather than implying a separate Stripe payment per child.
- Stripe remains the receipt renderer and source for payment/refund status. Common Time provides the searchable index and authorization layer. Owners can retrieve receipts without leaving the app; payers can retrieve only their own receipts through authenticated/magic-link portal access. Receipt lookup failure must fall back to local payment facts and a resend/contact path rather than losing history.
- Failed and declined Stripe charges require a first-class recovery workflow. Persist the attempt and normalized provider failure category, safe decline/message details, next-action eligibility, and webhook reconciliation without exposing sensitive provider internals. Notify the owner in-app and through their configured operational channel; show the affected family, statement, amount, attempt time, and a plain-language next step.
- Recovery actions must be state-aware: retry only when Stripe and the existing authorization permit it; request a new payment method when required; return to payer review if the amount changes; allow an owner to record cash/manual settlement, apply an account credit, schedule a follow-up, or explicitly write off the balance. Never silently loop off-session retries, duplicate a PaymentIntent, mark a failed attempt paid, or treat an owner notification as payer consent.
- Add an owner failed-payment queue with open, action-needed, retry-scheduled, awaiting-payer, resolved, and written-off states. Provider webhook events may advance payment truth, while owner workflow state and notes remain separately auditable. Notify on initial failure and material state change without creating duplicate alerts for replayed events.
- Add idempotent bulk draft preparation for a selected service month and explicit school/family scope. Each family is its own transactional unit: one invalid family must report its blocker without rolling back or concealing successfully prepared drafts for other families. Re-running the batch refreshes only eligible mutable drafts and never alters locked/submitted/approved periods.
...
- Live before-service acceptance passed with Kim family: four scheduled August lessons prepared and locked at $160 without requiring a past-service outcome. The payer email arrived, and the payer selected `A credit or discount is missing` with the note `The statement is missing the credit we discussed.` The request became rejected, its approval controls disappeared, no payment started, the period returned to editable review, and the owner saw the normalized reason and exact note.
- Payer responses require durable owner notification. Create an in-app notification record in the same successful response workflow, surface it through an owner inbox and immediate toast when active, and send transactional email as the minimum external alert. SMS is optional only for schools with the paid, isolated SMS add-on. Notification delivery failure must not roll back or hide the payer response; retries and delivery state are recorded separately and deduplicated.
- Kim replacement acceptance confirmed immutable request versioning: version 1 remains rejected at $160 and version 2 is independently pending at $150 with a different token hash. Never reuse a bearer URL for a revised amount. An old rejected/cancelled/superseded page should explain that the proposal is no longer actionable and that the school sent an updated statement; it must not reveal, embed, or automatically redirect to the new bearer token.
...
- Live approval acceptance passed later on 2026-08-18. A fresh $160 payer request became `approved` while `payment_status` remained `not_started`; the owner saw the Realtime toast and received the transactional email. Database evidence shows one durable `payer_approved` owner notice, subsequently read, and one matching outbox item accepted by Resend and reconciled to `delivered` approximately two seconds after the payer decision. Forced-provider-failure isolation and manual retry remain the open notification acceptance cases.
- The forced-failure setup first invalidated Resend before sending the payer approval request, exposing a distinct recovery gap from owner-notification retry. The database correctly preserved a pending approval request with a failed initial `email_deliveries` attempt, but the UI offered `Replace approval link` because it selected the action from request status alone.
- Added the composite `pending request + failed latest delivery` UI state and a dedicated payer approval-email retry transition. The database function locks and authorizes the request, requires the latest attempt to be failed, refuses suppressed recipients, preserves request ID/version/snapshot/amount, rotates the undelivered token and expiration, and creates an incremented delivery attempt with a new stable idempotency key. Replacement remains reserved for superseding the proposal.
- Raw bearer tokens remain intentionally unpersisted, so retry cannot reconstruct the failed email's link. Token rotation within the same request invalidates the unseen prior token without falsely creating a new financial proposal version. TypeScript, ESLint, and the webpack production build pass.
- Migrations `20260818143000` and corrective `20260818144000` are deployed. Linked lint caught and the follow-up corrected an ambiguous `recipient_email` reference before live use; linked lint now reports no errors. The pre-existing unused `school_name` warning in the owner-notification queue remains. Live retry acceptance remains.
- Martin family exposed the recipient-correction branch: its August $220 request remained pending while Resend correctly refused the seeded `elise.martin@example.com` address. Three retry attempts failed with provider `validation_error`; the locked draft remained intact. The payer email was then corrected to a reachable address.
- Failed-delivery UI now keeps `Retry original email` for unchanged recipients and adds `Send to updated payer email` for corrected addresses. Updated-recipient send uses normal atomic request supersession, preserving the locked statement while cancelling the unreachable token; a new draft is neither required nor permitted.
- Payer contact changes are now guarded at both UI and database layers. Editing an address while any family approval is pending shows an inline warning and explicit confirmation; the save transaction updates the contact, cancels every pending bearer request for that billing account, appends cancellation evidence with reason `payer_email_changed`, and audits hashes rather than raw old/new addresses. The owner is then directed to send again to the new address. Migration `20260818145000` is deployed and linked lint reports no new issues.
- Martin's $220 approval then passed the forced owner-email-failure case: payer approval persisted as `approved`, payment remained `not_started`, the owner received the Realtime toast, and the owner email outbox independently recorded Resend `validation_error: API key is invalid` as `failed`.
- Manual owner-email retry no longer relies on instructions not to click repeatedly. Migration `20260818150000` adds a database-locked retry claim, retry count, next-eligible time, escalating cooldown, and audit event; migration `20260818151000` sets the manual retry ceiling to five. The UI disables cooling-down or exhausted retries and displays the next eligible time. Accepted mail ceases to be retryable, concurrent/stale claims fail, and the stable provider idempotency key continues to protect the external side effect.
- Live recovery passed after restoring the valid Resend key: the failed Martin owner-notification outbox row was claimed once, reached `accepted`, and reconciled to `delivered` approximately one second later with `retry_count = 1`. The original $220 request remained approved with payment not started. Owner-facing success copy now says that the retry was accepted and delivery status will update, rather than exposing the provider handoff implementation.
- Added a durable, deduplicated platform support incident for failed owner-notification emails. Owner/admin reports are tenant-authorized, classified as temporary/configuration/recipient/suppression/unknown, auditable, and visible as `Problem reported` after submission. Diagnostics contain source/request/notification IDs, provider error code, retry count, failure time, and deployment reference; they exclude approval tokens, message bodies, recipient addresses, and credentials. The incident queue does not depend on email delivery. Migration `20260818152000` is deployed and linked lint reports no new issues; live report submission remains open until another failed item exists.
- Deferred acceptance TODO: use the next genuine or controlled failed owner-notification email to verify report creation, classification, safe diagnostics, owner-visible reported state, and duplicate submission deduplication. This is non-blocking for subsequent product work because it cannot mutate payer decisions, billing periods, or payment state.
...
### One-time lesson payment source review

Recorded on 2026-09-17.

- Scope: payer-present QR/link Checkout for one immutable per-session lesson price, connected-account direct charge, signed webhook completion, and zero-dollar allocation on a later family statement.
- Authorization is resolved server-side from signed claims plus `school.billing.manage`; the service-role client receives only server-derived school, lesson, snapshot, billing-account, product, and connection identifiers. The Server Action has an explicit durable throttle.
- The server retrieves the connected account's current Stripe Price and requires active, one-time, exact amount, exact currency, and exact account agreement before creating Checkout. Checkout is card-only for this first path and uses a durable request ID as both internal binding and Stripe idempotency identity.
- Review fixed four pre-commit integrity defects: exact 30-minute expiry clock skew, duplicate-risk retry of a provider-ambiguous `created` request, webhook dependence on a possibly failed local session-ID write, and persisted hosted receipt URLs. Expiry is now 35 minutes; retries reuse the request/key; webhooks recover through signed Checkout metadata and recheck account/amount/PaymentIntent/Charge; receipts remain provider-retrieved future work.
- Migration assertions require RLS, anonymous denial, authenticated mutation denial, service-only completion, and billing-draft awareness of separately paid lessons. Dependency audit reports zero known production vulnerabilities; TypeScript passes; linked pre-migration database lint has only the two existing unused-variable warnings.
- Decision: source-ready to commit, **not deployment-ready**. Local migration replay was unavailable because Docker was not running. Apply/rehearse only in Stripe test mode, then test success, cancellation, expiry, duplicate/concurrent initiation, provider acceptance plus local-write failure, webhook replay/order, wrong account/amount, draft regeneration, and receipt retrieval before enabling the control in production.
