...
type ProposalApproval = { kind:"schedule_proposal";id:string;status:string;teacherId:string;studentId:string;teacher:string;student:string;original:string|null;proposed:string;reason:string;createdAt:string };
...
export async function loadOwnerApprovals(client:Client,schoolId:string,{teacherId,studentIds,history=false,limit}:{teacherId?:string;studentIds?:string[];history?:boolean;limit?:number}={}){
 ...
 const filteredRequests=(requestRows??[]).filter(row=>{const event=eventMap.get(row.lesson_event_id);return event&&(!teacherId||event.teacher_id===teacherId)&&(!studentIds||studentIds.includes(event.student_id));});
 const ids=[...new Set([...(proposalRows??[]).flatMap(row=>[row.teacher_id,row.student_id]),...filteredRequests.flatMap(row=>{const event=eventMap.get(row.lesson_event_id);return event?[event.teacher_id,event.student_id]:[];})])];
 const versionIds=[...new Set(filteredRequests.map(row=>row.policy_version_id))];
 const [{data:people,error:peopleError},{data:policyOutcomes,error:policyError}]=await Promise.all([ids.length?client.from("people").select("id,first_name,last_name,preferred_name").eq("school_id",schoolId).in("id",ids):Promise.resolve({data:[],error:null}),versionIds.length?client.from("cancellation_policy_outcomes").select("id,policy_version_id,scenario,timing_bucket,calendar_action,original_charge_treatment,replacement_kind,expiration_days,adjustment_kind,adjustment_amount_cents").in("policy_version_id",versionIds):Promise.resolve({data:[],error:null})]);
 if(peopleError)throw new Error(`Approval people could not load: ${peopleError.message}`);if(policyError)throw new Error(`Approval policy outcomes could not load: ${policyError.message}`);
 const names=new Map((people??[]).map(person=>[person.id,`${person.preferred_name||person.first_name} ${person.last_name}`]));const outcomeMap=new Map((policyOutcomes??[]).map(outcome=>[`${outcome.policy_version_id}:${outcome.scenario}:${outcome.timing_bucket}`,outcome]));
 const proposals:ProposalApproval[]=(proposalRows??[]).map(row=>({kind:"schedule_proposal",id:row.id,status:row.status,teacherId:row.teacher_id,studentId:row.student_id,teacher:names.get(row.teacher_id)??"Teacher",student:names.get(row.student_id)??"Student",original:row.original_starts_at,proposed:row.proposed_starts_at,reason:row.reason,createdAt:row.created_at}));
