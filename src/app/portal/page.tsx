...
export default async function ClientPortalPage() {
  ...
  if (accessState !== "ready") return <main className="mx-auto min-h-screen max-w-4xl px-5 py-10 sm:px-8 sm:py-16">
    <header className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end"><div><p className="text-sm text-brand">Family scheduling</p><h1 className="mt-3 font-display text-5xl">{accessState === "ambiguous" ? "Account needs attention" : "Portal not set up"}</h1></div><PortalSignOut label="Use a different email" /></header>
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-5 py-10 sm:px-8 sm:py-16">
    ...
    <div className="mt-section space-y-section">{lessons?.length ? Array.from(schools.entries()).map(([schoolId, school]) => <section key={schoolId}>
      <header className="mb-6"><h2 className="font-display text-3xl">{schools.size > 1 ? school.name : "Lesson calendar"}</h2><p className="mt-2 text-xs text-muted">Times shown in {school.timeZone}</p></header>
      ...
      <div className="ui-card p-5 sm:p-7"><LessonCalendar
        ...
        /></div>
    </section>) : <section><h2 className="font-display text-3xl">No upcoming lessons</h2><div className="ui-card mt-6 p-6 sm:p-8"><p className="text-sm leading-6 text-muted">No scheduled lessons were found for this family during the next three months.</p></div>{calendarAccounts?.map((account) => <div key={account.school_id} className="mt-8"><CalendarSubscription schoolId={account.school_id} schoolName={account.school_name} active={account.subscription_active} /></div>)}</section>}</div>
  </main>;
}
