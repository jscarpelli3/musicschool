...
export function CalendarSubscription({ schoolId, schoolName, active }: CalendarSubscriptionProps) {
  ...
  return <aside className="ui-card p-5 sm:p-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      {!feedUrl ? <button type="button" disabled={pending} onClick={subscribe} className="shrink-0 rounded-control border border-brand px-5 py-3 text-sm text-brand transition hover:-translate-y-px hover:bg-brand hover:text-canvas disabled:opacity-50">{pending ? "Creating…" : active ? "Get a new private link" : "Subscribe to calendar"}</button> : null}
    </div>
    {feedUrl ? <div className="mt-5 border-l-2 border-brand pl-4">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <button type="button" onClick={openCalendar} className="rounded-control border border-brand bg-brand px-4 py-2 text-sm text-canvas">Open calendar app</button>
        <button type="button" onClick={copyLink} className="rounded-control border border-line bg-surface px-4 py-2 text-sm text-muted hover:border-brand hover:text-ink">Copy private link</button>
      </div>
    ...
    {active ? <button type="button" disabled={pending} onClick={revoke} className="text-action mt-4 text-xs text-muted hover:text-ink disabled:opacity-50">Remove calendar access</button> : null}
