...
export function AutomaticPaymentSettings(props: AutomaticPaymentSettingsProps) {
  ...
  return <section className="ui-card p-5 ring-1 ring-brand/40 sm:p-6">
    ...
    <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
      <div className="rounded-control bg-surface p-4"><dt className="text-xs text-muted">Monthly limit</dt><dd className="mt-1">{cap}</dd></div>
      <div className="rounded-control bg-surface p-4"><dt className="text-xs text-muted">Accepted</dt><dd className="mt-1">{new Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(new Date(props.acceptedAt))}</dd></div>
    </dl>
