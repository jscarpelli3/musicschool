...
export async function addStudentAndPayer(schoolId: string, _state: AddFamilyState, formData: FormData): Promise<AddFamilyState> {
  ...
  if (Object.values(values).some((value) => !value) || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(values.email)) return { ok: false, message: "Complete the student and payer details, including a valid email address." };
  ...
  try {
    ...
    const { error } = await supabase.rpc("create_onboarding_student_and_payer", { p_school_id: schoolId, p_student_first_name: values.studentFirst, p_student_last_name: values.studentLast, p_payer_first_name: values.payerFirst, p_payer_last_name: values.payerLast, p_payer_email: values.email, p_payer_profile_id: payerProfileId, p_relationship: values.relationship });
