...
export async function resolveLessonChangeRequest(schoolId:string,requestId:string,input:{decision:"approved"|"declined";lessonResolution:string|null;adjustmentKind:"fee"|"credit"|null;adjustmentAmountCents:number;reason:string}){
 ...
 if(error){const detail=error.message;const message=detail.includes("override_reason_required")?"Explain why this decision differs from the policy.":detail.includes("lesson_no_longer_scheduled")?"The lesson changed while you were reviewing it. Nothing was applied; reload to see its current state.":detail.includes("request_not_found")||detail.includes("not_authorized")?"That request is unavailable or you no longer have access.":"The request could not be resolved. Nothing changed, so it is safe to try again.";return{ok:false,message};}
