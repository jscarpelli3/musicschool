...
export function ProposalReview({schoolId,proposalId,status,student,teacher,original,proposed,reason}:{schoolId:string;proposalId:string;status:string;student:string;teacher:string;original:string;proposed:string;reason:string}){
 ...
 useEffect(()=>{const previous=document.body.style.overflow;const close=(event:KeyboardEvent)=>{if(event.key==="Escape")router.push(`/schools/${schoolId}/notifications`);};document.body.style.overflow="hidden";window.addEventListener("keydown",close);return()=>{document.body.style.overflow=previous;window.removeEventListener("keydown",close);};},[router,schoolId]);
 ...
 return <div className="fixed inset-0 z-[100]" role="dialog" aria-modal="true" aria-labelledby="proposal-review-title">
  <Link href={`/schools/${schoolId}/notifications`} aria-label="Close proposal review" className="fixed inset-0 bg-[var(--ui-overlay)]" />
  ...
  <div className="flex items-start justify-between gap-5"><div><p className="text-xs uppercase tracking-[0.14em] text-brand">Teacher reschedule proposal</p><h2 id="proposal-review-title" className="mt-3 font-display text-4xl">Review the proposed time.</h2></div><Link href={`/schools/${schoolId}/notifications`} className="text-sm text-muted hover:text-ink">Close</Link></div>
