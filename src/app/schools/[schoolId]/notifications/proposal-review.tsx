...
export function ProposalReview({schoolId,proposalId,status,studentId,student,teacherId,teacher,original,proposed,reason,closeHref=`/schools/${schoolId}/notifications`}:{schoolId:string;proposalId:string;status:string;studentId:string;student:string;teacherId:string;teacher:string;original:string;proposed:string;reason:string;closeHref?:string}){
 ...
 const pending=lessonProposalDescriptor(status).reviewable===true;
 return <div className="fixed inset-0 z-[100]" role="dialog" aria-modal="true" aria-labelledby="proposal-review-title">
  ...
  <p className="mt-4 text-sm leading-6 text-muted"><Link href={`/schools/${schoolId}/students/${studentId}`} className="hover:text-brand">{student}</Link> remains at the original time unless you approve this proposal from <Link href={`/schools/${schoolId}/staff/${teacherId}`} className="hover:text-brand">{teacher}</Link>.</p>
  <dl className="mt-7 grid gap-px bg-line sm:grid-cols-2"><div className="bg-canvas p-5"><dt className="text-xs text-muted">Teacher</dt><dd className="mt-2 text-sm font-medium"><Link href={`/schools/${schoolId}/staff/${teacherId}`} className="hover:text-brand">{teacher}</Link></dd></div><div className="bg-canvas p-5"><dt className="text-xs text-muted">Student</dt><dd className="mt-2 text-sm font-medium"><Link href={`/schools/${schoolId}/students/${studentId}`} className="hover:text-brand">{student}</Link></dd></div></dl>
