...
export function OwnerNotifications({ schoolId, embedded = false }: { schoolId: string; embedded?: boolean }) {
  ...
  return <>
    <div className={embedded ? "notification-control relative z-[90] flex flex-col items-center" : "fixed right-5 top-5 z-[90]"}>
      <button type="button" aria-expanded={open} onClick={() => setOpen((value) => { if (!value) window.dispatchEvent(new Event("common-time:open-notifications")); return !value; })} className={`rounded-control px-3 py-1.5 text-sm transition ${unread ? "bg-brand text-canvas" : "text-brand hover:bg-surface"}`}>Notifications{unread ? <span className="ml-2 inline-grid min-w-5 place-items-center rounded-full border border-canvas/50 px-1 text-xs" aria-label={`${unread} unread notifications`}>{unread}</span> : null}</button>
      {embedded ? <Link href={`/schools/${schoolId}/notifications`} className="mt-1 text-xs text-muted hover:text-ink">View all</Link> : null}
