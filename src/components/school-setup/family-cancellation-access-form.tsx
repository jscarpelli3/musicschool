...
export function FamilyCancellationAccessForm({initial,action}:{initial:{timelyApprovalMode:string;refundPortalMode:string};action:(previous:CancellationPolicyState,data:FormData)=>Promise<CancellationPolicyState>}){
 ...
 return <form action={formAction} className="space-y-7">
  <label className="block"><span className="text-sm font-medium">Timely cancellations</span><span className="mt-1 block text-xs leading-5 text-muted">Choose whether an eligible family action changes the lesson immediately or waits for owner review.</span><select name="timely_approval_mode" value={timelyApprovalMode} onChange={(event)=>setTimelyApprovalMode(event.target.value)} className={field}><option value="owner_review">Owner review required</option><option value="automatic" disabled>Automatically approve · activation pending transaction rehearsal</option></select></label>
