...
export function TeacherRescheduleControls({
  ...
  return (
    <section className="mt-6 rounded-control bg-surface-raised p-5">
      ...
      <div className="mt-4">
          ...
          <label className="mt-4 block"><span className="text-xs text-muted">New date and time</span><input type="datetime-local" min={earliestLocal} value={localStart} onChange={(event) => setLocalStart(event.target.value)} className="mt-2 w-full rounded-control border border-line bg-surface px-4 py-3 outline-none focus:border-brand" /></label>
