...
begin
 ...
 insert into public.audit_log(school_id,actor_profile_id,action,entity_type,entity_id,metadata) values(p_school_id,actor_id,'school.student_and_payer_created','student',student_id,jsonb_build_object('billing_account_id',account_id,'payer_person_id',payer_id));
 return jsonb_build_object('student_id',student_id,'billing_account_id',account_id);
end;$$;

revoke all on function public.create_student_and_payer(uuid,text,text,text,text,text,uuid,text) from public,anon;
grant execute on function public.create_student_and_payer(uuid,text,text,text,text,text,uuid,text) to authenticated;

create or replace function public.create_onboarding_student_and_payer(
 p_school_id uuid,p_student_first_name text,p_student_last_name text,
 p_payer_first_name text,p_payer_last_name text,p_payer_email text,p_payer_profile_id uuid,
 p_relationship text default 'parent'
) returns jsonb language plpgsql security definer set search_path='' as $$
begin
 ...
 return public.create_student_and_payer(p_school_id,p_student_first_name,p_student_last_name,p_payer_first_name,p_payer_last_name,p_payer_email,p_payer_profile_id,p_relationship);
end;$$;
