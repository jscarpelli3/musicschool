alter table public.billing_statement_notice_deliveries add column attempt_number integer not null default 1 check(attempt_number>0);
do $$
declare constraint_name text;
begin
  select constraint_row.conname into constraint_name from pg_constraint constraint_row
  where constraint_row.conrelid='public.billing_statement_notice_deliveries'::regclass
    and constraint_row.contype='u'
    and pg_get_constraintdef(constraint_row.oid)='UNIQUE (billing_period_id, mandate_id)';
  if constraint_name is null then raise exception 'logical notice uniqueness constraint not found'; end if;
  execute format('alter table public.billing_statement_notice_deliveries drop constraint %I',constraint_name);
end;
$$;
alter table public.billing_statement_notice_deliveries add constraint billing_statement_notice_attempt_unique
unique(billing_period_id,mandate_id,attempt_number);
...
declare actor_id uuid:=auth.uid(); readiness_row record; period_row public.billing_periods%rowtype;
  ...
  delivery_key text; next_attempt integer;
begin
  ...
  if exists(select 1 from public.billing_statement_notice_deliveries notice where notice.billing_period_id=period_row.id
    and notice.mandate_id=mandate_row.id and notice.status not in ('failed','bounced','complained','suppressed','cancelled'))
  ...
  values(p_school_id,actor_id,'billing_statement_notice.prepared','billing_period',period_row.id,
    jsonb_build_object('notice_delivery_id',delivery_id,'mandate_id',mandate_row.id,'attempt_number',next_attempt,
      'amount_cents',period_row.amount_due_cents,'notice_days',mandate_row.advance_notice_days));
...
comment on column public.billing_statement_notice_deliveries.attempt_number is
  'Immutable provider-attempt sequence. Recipient, content hash, provider identity, and idempotency key are never overwritten by retry.';
