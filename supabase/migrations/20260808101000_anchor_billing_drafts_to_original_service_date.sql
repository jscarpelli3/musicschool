-- Refresh the draft generator so operational rescheduling cannot move or
-- duplicate a lesson's financial month.
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    p_school_id::text || ':' || p_billing_account_id::text || ':' || month_start::text, 0
  ));
  ...
  where billing_period_id = period_id and source_type in ('lesson','lesson_series');

  for series_row in
    select series.id as lesson_series_id, series.starts_on, series.ends_on,
      term.id as billing_terms_id, term.amount_cents, term.currency,
      term.offering_name, term.effective_from, term.effective_until
    from public.lesson_series series
    join public.billing_account_students mapping
      on mapping.school_id = series.school_id and mapping.student_id = series.student_id
      and mapping.billing_account_id = p_billing_account_id
    join public.lesson_series_billing_terms term
      on term.school_id = series.school_id and term.lesson_series_id = series.id
      and term.billing_mode = 'fixed_monthly' and term.effective_from <= month_start
  ...
  for event_row in
    select event.id as lesson_event_id, event.starts_at as operational_starts_at,
      event.lesson_series_id, snapshot.id as snapshot_id,
      snapshot.series_billing_terms_id, snapshot.billing_mode,
      snapshot.amount_cents, snapshot.currency, snapshot.offering_name,
      snapshot.billing_service_date as service_date
    ...
    join public.lesson_event_price_snapshots snapshot
      on snapshot.school_id = event.school_id and snapshot.lesson_event_id = event.id
    where event.school_id = p_school_id
      and snapshot.billing_service_date between month_start and month_end
  ...
  loop
    ...
    if result ->> 'state' = 'not_ready' then
      disposition := 'charge'; reason_code := 'scheduled_upcoming';
    else
    ...
    insert into public.billing_line_items (
      ...
      jsonb_build_object(
        'billing_mode', event_row.billing_mode, 'disposition', disposition,
        ...
        'operational_starts_at', event_row.operational_starts_at
      ),
